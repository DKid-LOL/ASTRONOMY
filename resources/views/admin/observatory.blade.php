...
<body class="bg-gray-900 text-white">
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-4">
            <h1 class="text-xl font-bold pb-2 mb-6 border-b border-gray-700">List of Observatory</h1>
            <ul class="space-y-4">
                @foreach ($observatories as $observatory)
                    <li class="bg-gray-800 rounded-lg p-4 hover:bg-gray-700 flex justify-between items-center">
                        <div class="flex items-center space-x-4">
                            <img src="{{ asset($observatory->image) }}" alt="{{ $observatory->name }}" class="w-20 h-20 rounded-full">
                            <span class="text-lg font-medium">{{ $observatory->name }}</span>
                        </div>
                        <div class="flex items-center space-x-2">
                            <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                <a href="{{ url('update_observatory', $observatory->id) }}">Edit</a>
                            </button>
                            <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                                <a href="{{ url('delete_observatory', $observatory->id) }}">Delete</a>
                            </button>
                        </div>
                    </li>
                @endforeach
